...
function initializeMetronomeUI() {
  ...
  const presetButtons = document.querySelectorAll('.preset-button'); // Default presets
  const savePresetBtn = document.getElementById('save-preset'); // Button to OPEN save modal
  ...
  const presetSaveBtn = document.getElementById('preset-save'); // Button inside modal to SAVE
  ...
  const presetsGrid = document.getElementById('presets-grid'); // Grid for default + user presets
  const presetForm = document.getElementById('preset-form'); // Form element

  // Preset Modal - Include Checkboxes
  ...
  // Preset Modal - Controls within Save/Edit Tab
  ...
  // Add any other elements needed from the original lines 468-493 if missed

  // Alert/Confirm Modal Elements (Ensure these IDs exist in your HTML)
  ...
  function handlePresetSoundButtonClick() {
    ...
    document.querySelectorAll('.preset-sound-button').forEach(b => {
  ...
  // Listener to OPEN the preset modal
  const mainSavePresetBtn = document.getElementById('save-preset'); // Assuming this is the button OUTSIDE the modal
  if (mainSavePresetBtn) {
      mainSavePresetBtn.addEventListener('click', () => {
  ...
  if (tapButton) {
    tapButton.addEventListener('click', () => {
      ...
      tapTimeout = setTimeout(() => {
        tapTimes = []
        if (tapDisplay) tapDisplay.textContent = "-- BPM"
      }, 2000)
  ...
  if (useClickSubdivisionCheckbox) {
  ...
  // Default Preset Buttons Listener
  const defaultPresetButtons = document.querySelectorAll('.preset-button:not(.user-preset)'); // Select only default ones
  if (defaultPresetButtons) {
      defaultPresetButtons.forEach(btn => {
         btn.addEventListener('click', () => {
            ...
            // Reset other things potentially? Or assume default presets only set tempo/time sig?
            // Example: Reset subdivision if not specified by preset
            // if (!btn.dataset.subdivision && subdivisionSelector) {
            //     subdivision = parseInt(subdivisionSelector.options[0].value); // Reset to default
  ...
  window.addEventListener('click', (e) => {
      if (presetModal && e.target === presetModal) {
  ...
  window.addEventListener('keydown', (e) => {
      ...
      if (e.code === 'Enter') {
          if (isTempoInputFocused) {
              if (tempoDisplay) tempoDisplay.blur();
              e.preventDefault();
          } else if (!isInputFocused && tempoDisplay) {
              tempoDisplay.focus();
              tempoDisplay.select();
              e.preventDefault();
          }
  ...
  if (presetTabs) {
      presetTabs.forEach(tab => {
          tab.addEventListener('click', () => {
              ...
              if (tabId === 'save' && editTabButtons && editTabButtons.style.display === 'none') {
                  resetPresetForm();
                  initializePresetControls();
                  if (includeAccentPatternCheck && includeAccentPatternCheck.checked) { updatePresetAccentPattern(); }
              } else if (tabId === 'my-presets') {
                  renderUserPresets(); // Refresh the list when switching to it
              }
          });
      });
  }
  ...
  if (modalSaveButton && modalSaveButton.tagName === 'BUTTON') {
      modalSaveButton.addEventListener('click', async () => {
          ...
          if (checks.every(chk => !chk || !chk.checked)) {
              showAlert('Please select at least one setting to include in the preset');
              return;
          }
          ...
          const preset = {
              ...
              name: name,
              description: presetDescInput ? presetDescInput.value.trim() : '',
          ...
          if (includeTimeSignatureCheck && includeTimeSignatureCheck.checked && presetNumEl && presetDenEl) {
              preset.settings.beatsPerMeasure = parseInt(presetNumEl.textContent);
              preset.settings.noteValue = parseInt(presetDenEl.textContent);
          }
  ...
  if (presetUpdateBtn) {
      presetUpdateBtn.addEventListener('click', async () => {
          ...
          const presetDenEl = document.getElementById('preset-time-sig-denominator');
          if (includeTimeSignatureCheck && includeTimeSignatureCheck.checked && presetNumEl && presetDenEl) {
              updatedPreset.settings.beatsPerMeasure = parseInt(presetNumEl.textContent);
              updatedPreset.settings.noteValue = parseInt(presetDenEl.textContent);
          }
           const presetSubdivisionSelectorEl = document.getElementById('preset-subdivision-selector');
  ...
  function resetPresetForm() {
      ...
      initializePresetControls();
       ...
       if (container && includeAccentPatternCheck && !includeAccentPatternCheck.checked) {
  ...
  function showAlert(message, callback) {
      if (!alertModal || !alertMessage || !alertConfirm) {
          console.warn("Alert modal elements not found. Alert:", message);
  ...
  function showConfirm(message, onConfirm) {
       if (!confirmModal || !confirmMessage || !confirmOk || !confirmCancel) {
           console.warn("Confirm modal elements not found. Confirm:", message);
           return;
       }
  ...
  function initializePresetControls(preset = null) {
      ...
      const presetSoundButtonsNodeList = document.querySelectorAll('.preset-sound-button'); // Get NodeList
      if (presetSoundButtonsNodeList) {
          presetSoundButtonsNodeList.forEach(btn => {
              if (!btn) return;
              btn.classList.remove('selected');
              if (btn.dataset.sound === sourceSound) {
                  btn.classList.add('selected');
              }
              btn.removeEventListener('click', handlePresetSoundButtonClick);
              btn.addEventListener('click', handlePresetSoundButtonClick);
          });
      }
      ...
      if (presetDecreaseBeats) {
          presetDecreaseBeats.removeEventListener('click', handlePresetDecreaseBeats);
          presetDecreaseBeats.addEventListener('click', handlePresetDecreaseBeats);
      }
       if (presetIncreaseBeats) {
           presetIncreaseBeats.removeEventListener('click', handlePresetIncreaseBeats);
           presetIncreaseBeats.addEventListener('click', handlePresetIncreaseBeats);
       }
       if (presetDecreaseNoteValue) {
           presetDecreaseNoteValue.removeEventListener('click', handlePresetDecreaseNoteValue);
           presetDecreaseNoteValue.addEventListener('click', handlePresetDecreaseNoteValue);
       }
       if (presetIncreaseNoteValue) {
           presetIncreaseNoteValue.removeEventListener('click', handlePresetIncreaseNoteValue);
           presetIncreaseNoteValue.addEventListener('click', handlePresetIncreaseNoteValue);
       }
  }
