 ...
 * Temperament engine
 * -------------------------------------------------------
 * - All calculations are anchored to A4 = referencePitch.
 * - For non-ET systems we compute exact pitch-class offsets
 *   in cents relative to ET with A (pc=9) forced to 0¢,
 *   then build center frequencies f = fET * 2^(offset/1200)
 *   for each MIDI number.
 * - Note selection: pick the nearest center in cents.
 */

type TunerState = {
  isActive: boolean;
  referencePitch: number; // A4
  transpose: number;      // written = concert + transpose
  useFlats: boolean;
...
type TemperamentId = 'equal' | 'pythagorean' | 'meantone' | 'just';

type Temperament = {
  id: TemperamentId;
  name: string;
  offsetsA: number[]; // length 12, cents offsets for pc 0..11 with A(pc=9)=0
};
...
function buildTemperament(id: TemperamentId): Temperament {
  ...
  if (id === 'pythagorean') {
    ...
    const kByPc: Record<number, number> = {
      9: 0,   // A
      4: 1,   // E
      11: 2,  // B
      6: 3,   // F#
    ...
    const off: number[] = new Array(12).fill(0);
...
class PitchCenters {
  private centers = new Map<number, number>(); // midi -> freq
  private refA4 = 440;
  private offsetsA: number[] = new Array(12).fill(0);

  configure(refA4: number, offsetsA: number[]) {
    this.refA4 = refA4;
  ...
  nearest(f: number): { midi: number; freq: number; cents: number } | null {
...
class TunerUI {
  ...
  // pre-rendered gauge background to minimize redraw work
  private gaugeBG?: HTMLCanvasElement;

  private centers = new PitchCenters();
  private temperament = buildTemperament('equal');
  ...
  // UI elements
  private elements = {
    startBtn: document.getElementById('start-tuner') as HTMLButtonElement,
    stopBtn: document.getElementById('stop-tuner') as HTMLButtonElement,
    noteName: document.getElementById('note-name') as HTMLDivElement,
    noteOctave: document.getElementById('note-octave') as HTMLDivElement,
    centsDisplay: document.getElementById('cents-display') as HTMLDivElement,
    frequencyReadout: document.getElementById('frequency-readout') as HTMLDivElement,
    clarityReadout: document.getElementById('clarity-readout') as HTMLDivElement,
    centsBarIndicator: document.getElementById('cents-bar-indicator') as HTMLDivElement,
    statusText: document.getElementById('status-text') as HTMLSpanElement,
    statusDot: document.querySelector('.status-dot') as HTMLDivElement,
    referencePitch: document.getElementById('reference-pitch') as HTMLInputElement,
    referenceDisplay: document.getElementById('reference-display') as HTMLDivElement,
    transpose: document.getElementById('transpose') as HTMLSelectElement,
    temperament: document.getElementById('temperament') as HTMLSelectElement,
  };

  // smoothing / display cadence
  private readonly MIN_CLARITY_FOR_LOCK = 0.75;
  private readonly TEXT_UPDATE_MS = 40; // 25 Hz for text is enough
  ...
  constructor() {
    this.canvas = document.getElementById('tuner-gauge') as HTMLCanvasElement;
  ...
  private setupEventListeners(): void {
    ...
    this.elements.referencePitch.addEventListener('input', (e) => {
      ...
      if (!Number.isNaN(val)) setRef(val);
    });
    ...
    const setTranspose = (s: string) => { this.state.transpose = parseInt(s, 10); };
    this.elements.transpose.addEventListener('change', (e) => setTranspose((e.target as HTMLSelectElement).value));
    document.querySelector('[data-action="decrease-transpose"]')?.addEventListener('click', () => {
      const select = this.elements.transpose;
      if (select.selectedIndex > 0) { select.selectedIndex -= 1; setTranspose(select.value); }
    });
    document.querySelector('[data-action="increase-transpose"]')?.addEventListener('click', () => {
      const select = this.elements.transpose;
      if (select.selectedIndex < select.options.length - 1) { select.selectedIndex += 1; setTranspose(select.value); }
    });
    ...
    this.elements.temperament.addEventListener('change', (e) => {
      this.state.temperament = (e.target as HTMLSelectElement).value as TemperamentId;
      this.rebuildTemperament();
    });
  }

  private rebuildTemperament() {
  ...
  private buildGaugeBackground() {
    ...
    const drawArc = (startCents: number, endCents: number, color: string, alpha: number) => {
  ...
  private async start(): Promise<void> {
    try {
      ...
      // pitch frames
      this.detector.addEventListener('pitch', ((e: CustomEvent<PitchFrame>) => {
        this.onPitch(e.detail);
      }) as EventListener);

      // errors
      this.detector.addEventListener('error', ((e: CustomEvent<Error>) => {
  ...
  private onPitch(frame: PitchFrame): void {
    ...
    if (this.centsHistory.length > 5) this.centsHistory.shift();
  ...
  private getCenterFreq(midi: number): number {
  ...
  private computeDisplayFromMidi(concertMidi: number, transpose: number): { note: string; octave: number } {
  ...
  private updateNoteDisplay(note: string, octave: number, cents: number, clarity: number): void {
    // Text
    ...
    if (absC <= 3) colorClass = 'in-tune';
    else if (absC <= 12) colorClass = 'close';
  ...
  /** Gauge drawing */
  private drawGauge(cents: number): void {
    const { width, height } = this.canvas;
    this.ctx.clearRect(0, 0, width, height);
    if (this.gaugeBG) this.ctx.drawImage(this.gaugeBG, 0, 0);
  ...
  private centsToAngle(cents: number): number {
  ...
  private updateStatus(text: string, active: boolean): void {
    ...
    if (active) this.elements.statusDot.classList.add('active');
    else this.elements.statusDot.classList.remove('active');
  }
}
