const R = (min, v, max) => Math.max(min, Math.min(max, v));
export class SongRecorder {
    ...
    setTempo(bpm) { this.opts.tempo = R(40, Math.round(bpm), 240); }
    setGrid(grid) { this.opts.grid = grid; }
    setSwing(amount) { this.opts.swing = R(0, amount, 0.6); }
    start(t0Seconds) {
        this.isRec = true;
        this.frames = [];
        this.recorded = [];
    ...
    segment() {
        ...
        for (let i = 0; i < this.frames.length; i++) {
            ...
            let midi = f.midi;
            if (this.opts.scale)
                midi = this.snapToScale(midi, this.opts.scale);
            if (!current) {
                // Start first note
    ...
    play(ctx, when = ctx.currentTime + 0.05) {
