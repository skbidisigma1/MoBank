...
type PitchWorkletConfig = {
  bufferSize?: number;     // analysis window (samples)
  hopSize?: number;        // stride per analysis
  minFreq?: number;        // Hz
  maxFreq?: number;        // Hz
  yinThreshold?: number;   // typical 0.1..0.2
  rmsFloorDb?: number;     // voice gate floor (e.g., -60 dBFS)
  clarityFloor?: number;   // 0..1 minimal clarity to emit
  hangMs?: number;         // keep "voiced" a bit after drop
  dcBlock?: boolean;       // DC blocker
};

const DEFAULTS: Required<PitchWorkletConfig> = {
  bufferSize: 2048,
  hopSize: 256,
  minFreq: 80,
  maxFreq: 1000,
...
class PitchProcessor extends AudioWorkletProcessor {
  private cfg: Required<PitchWorkletConfig>;
  private ring: Float32Array;
  private rHead = 0;
  private have = 0;
  private samplesProcessed = 0;
  private lastVoicedUntil = 0; // seconds
  private prevX = 0; // for DC block
  private prevY = 0;
  ...
  process(inputs: Float32Array[][]) {
    ...
    for (let i = 0; i < n; i++) {
      let x = 0;
      for (let c = 0; c < input.length; c++) x += (input[c]?.[i] ?? 0);
      x /= input.length || 1;
    ...
    while (this.have >= this.cfg.bufferSize) {
      ...
      if (voicedNow) this.lastVoicedUntil = t + this.cfg.hangMs / 1000;
  ...
  private readWindow(size: number): Float32Array {
  ...
  private analyzeFrame(buf: Float32Array, sr: number) {
    const N = buf.length;
    // RMS
    let s = 0, peak = 0;
    for (let i = 0; i < N; i++) { const v = buf[i]; s += v * v; peak = Math.max(peak, Math.abs(v)); }
  ...
  private yin(x: Float32Array, tauMin: number, tauMax: number) {
    const N = x.length;
  ...
  private yinPickTau(cmndf: Float32Array, tauMin: number, tauMax: number, threshold: number) {
    let tau = 0;
    ...
    if (tau === 0) {
      ...
      if (!isFinite(minVal)) return { tau: 0, clarity: 0 };
    }
